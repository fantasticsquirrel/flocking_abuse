  ...
  return (
    <section className="filters" aria-labelledby="filter-heading">
      ...
      <div className="filter-grid">
        ...
        <label><span>Source type</span><select aria-label="Source type" value={filters.sourceType} onChange={(event) => set('sourceType', event.target.value)}><option value="">All source types</option>{sourceTypes.map((type) => <option key={type} value={type}>{humanize(type)}</option>)}</select></label>
      </div>
    </section>
  );
}
