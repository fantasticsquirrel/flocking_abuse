...
export const canonicalSlug = (value: string): string => value.toLocaleLowerCase('en-US').normalize('NFKD').replace(/['’]/g, '').replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '').slice(0, 80);
export const canonicalLocation = (location: { city: string; county: string; state: string; country: string }): string =>
  [location.country, location.state, location.county || location.city].map(canonicalSlug).filter(Boolean).join('-');
...
export const IncidentSchema = z.object({
  ...
  if (parts.length !== 4 || parts.some((part) => !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(part))) {
    ...
    if (locationSegment !== expectedLocation) context.addIssue({ code: 'custom', path: ['uniqueness', 'canonical_key'], message: 'Canonical-key location must match the record location' });
    if (canonicalDateSegment !== expectedDate) context.addIssue({ code: 'custom', path: ['uniqueness', 'canonical_key'], message: 'Canonical-key date must match the occurrence month or unknown' });
    if (expectedActors.size === 0 || !expectedActors.has(actorSegment ?? '')) context.addIssue({ code: 'custom', path: ['uniqueness', 'canonical_key'], message: 'Canonical-key actor must match the main agency or entity' });
    if (!eventSegment || eventSegment.length < 4) context.addIssue({ code: 'custom', path: ['uniqueness', 'canonical_key'], message: 'Canonical key requires a factual event segment' });
  }
