...
export function readRuntimeConfig(env: NodeJS.ProcessEnv, cwd = process.cwd()): RuntimeConfig {
  ...
  if (!/^\$2[aby]\$\d{2}\$[^\s]{53}$/.test(passwordHash)) throw new Error('ADMIN_PASSWORD_HASH must be a bcrypt hash');
  ...
  if (!parsedPort.success) throw new Error('PORT must be an integer from 1 through 65535');
  ...
  if (!parsedOrigin.success) throw new Error(`APP_ORIGIN is invalid: ${parsedOrigin.error.issues.map((issue) => issue.message).join('; ')}`);
  if (nodeEnv === 'production' && new URL(parsedOrigin.data).protocol !== 'https:') throw new Error('APP_ORIGIN must use https in production');
  const releaseSha = nodeEnv === 'production' ? requireValue(env, 'RELEASE_SHA') : (env.RELEASE_SHA?.trim() || 'development');
  if (nodeEnv === 'production' && !/^[a-f0-9]{40}$/i.test(releaseSha)) throw new Error('RELEASE_SHA must be a full 40-character Git commit SHA');
  return {
    nodeEnv,
    host: '127.0.0.1',
...
export function createProductionApp(config: RuntimeConfig): express.Express {
  ...
  app.get('/docs/:document.html', async (request, response, next) => {
    try {
      ...
      if (!title) { response.status(404).json({ error: 'Not found' }); return; }
  ...
  app.use((request: Request, response: Response, next) => {
    if (request.method !== 'GET' || request.path.startsWith('/api/') || request.path.startsWith('/docs/')) { next(); return; }
    response.sendFile('index.html', { root: config.distDir, dotfiles: 'deny' }, (error) => {
      if (error) next(error);
    });
  });
