...
The deploy requires pre-provisioned mutable-data directories that pass `deploy/verify-data-permissions.sh`: no symlinks, special entries, or multiply linked files. Accepted, reported-unverified, runtime approval, and published-candidate archive records are root-owned and service-readable but not service-writable; the private `.delivered-candidates` review archive is root-only (`0700` directories and `0600` files); candidate and analytics directories are service-owned. Pre-provision `incidents`, `unverified`, `approvals`, and `published-candidates` as `root:flocking-abuse` mode `0750`, `.delivered-candidates` as `root:root` mode `0700`, and `candidates` and `analytics` as `flocking-abuse:flocking-abuse` mode `0750`. Analytics are atomically written to `analytics/analytics.json` mode `0600`; back up all mutable data, including runtime approvals and both candidate archives. Code deployment never changes or restores mutable-data ownership, modes, or contents. Any one-time metadata migration requires a separate root-only backup and explicit maintenance operation. The release takes an exclusive host deployment lock and automatically restores the prior release, both service units and their enablement/activity, nginx configuration, and enabled-site link if readiness or edge activation fails. Schema migrations require their own application-consistent backup and maintenance window.
