...
import { IncidentSchema, IncidentTypeSchema, SourceReliabilitySchema, SourceTypeSchema } from '../src/lib/incidentSchema.js';
...
const readSession = (request, options) => {
    ...
    try {
        ...
        if (typeof session.exp !== 'number' || typeof session.csrf !== 'string' || session.exp <= Math.floor((options.now?.() ?? new Date()).getTime() / 1000))
            return undefined;
        return session;
    }
    catch {
...
const setSessionCookie = (response, value, secure, maxAge = SESSION_TTL_SECONDS) => {
...
const requireOrigin = (options) => (request, response, next) => {
    if (request.get('Origin') !== options.allowedOrigin) {
        response.status(403).json({ error: 'Origin rejected' });
        return;
    }
    next();
};
const requireAdmin = (options) => (request, response, next) => {
...
async function writeCandidate(dataDir, incident) {
